...
namespace Thavyra.Rest.Features.Applications.Objectives.Get;
...
public class Endpoint : Endpoint<Request, ObjectiveResponse>
{
    ...
    public Endpoint(
        IRequestClient<Objective_GetById> client,
    ...
    public override void Configure()
    {
        ...
        Summary(x =>
        {
            x.Summary = "Get Scoreboard Objective";
        });
    }
    ...
    public override async Task HandleAsync(Request req, CancellationToken ct)
    {
        ...
        var authorizationResult = await _authorizationService.AuthorizeAsync(User, application,
            Security.Policies.Operation.Application.ReadObjectives);

        if (!authorizationResult.Succeeded)
        {
            await this.SendAuthorizationFailureAsync(authorizationResult.Failure, ct: ct);
            return;
        }
        ...
        if (objective.ApplicationId != application.Id)
        {
            await SendNotFoundAsync(ct);
            return;
        }
